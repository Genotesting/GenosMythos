...
function buildAlphabet(){
  ...
  alpha.forEach(letter=>{
    ...
    btn.addEventListener('keydown', (ev) => {
      if(ev.key === 'Enter' || ev.key === ' ') { ev.preventDefault(); searchByLetter(letter); }
    });
    el.appendChild(btn);
  });
}
...
function buildTagList(){
  ...
  favBtn.addEventListener('keydown', (ev) => {
    if(ev.key === 'Enter' || ev.key === ' ') { ev.preventDefault(); favBtn.click(); }
  });
...
function setupEvents(){
  const s = $('#search');
  if(s) s.addEventListener('input', debounce(()=>{ clearTagPressed(); filterToFavorites = false; visibleCount = PAGE_SIZE; renderList(); }, 180));
  const cf = $('#category-filter');
  if(cf) cf.addEventListener('change', ()=> { clearTagPressed(); filterToFavorites = false; visibleCount = PAGE_SIZE; renderList(); });
...
function showListView(){
  ...
  const m = $('#message'); if(m) m.hidden = true;
}
